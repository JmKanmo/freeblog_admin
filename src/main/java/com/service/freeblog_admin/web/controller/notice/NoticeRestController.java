...
import com.service.freeblog_admin.web.dto.notice.NoticePagingResponseDto;
...
public class NoticeRestController {
    ...
    public ResponseEntity<NoticePagingResponseDto> noticeList(Principal principal, @ModelAttribute NoticeSearchPagingDto noticeSearchPagingDto) {
        try {
            if (principal == null || principal.getName() == null) {
                throw new AdminException(ServiceExceptionMessage.NOT_LOGIN_STATUS_ACCESS.message());
            }
            return ResponseEntity.status(HttpStatus.OK).body(NoticePagingResponseDto.success(noticeService.findTotalPaginationNotice(noticeSearchPagingDto)));
        } catch (Exception exception) {
            if (BlogAdminUtil.getErrorMessage(exception) == ConstUtil.UNDEFINED_ERROR) {
                log.error("[freeblog_admin-noticeList] exception occurred ", exception);
            }
