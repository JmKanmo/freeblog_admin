class NoticeListController extends UtilController {
    ...
    initNoticeListController() {
        this.requestNoticeList(`/notice/list-page`);
    ...
    requestNoticeList(url, page) {
        ...
        xhr.addEventListener("loadend", event => {
            ...
            if (((status >= 400 && status <= 500) || (status > 500)) || (status > 500)) {
                ...
                if (responseValue["noticePaginationResponse"]["noticeTotalDto"]["totalCount"] <= 0) {
    ...
    #handleTemplateList(responseValue) {
        const noticeListTemplate = document.getElementById("notice-list-template").innerHTML;
        const noticeListTemplateObject = Handlebars.compile(noticeListTemplate);
        const jsonObj = responseValue["noticePaginationResponse"]["noticeTotalDto"]["noticeDtoList"];
        const noticeListTemplateHTML = noticeListTemplateObject({"noticeList": jsonObj});
